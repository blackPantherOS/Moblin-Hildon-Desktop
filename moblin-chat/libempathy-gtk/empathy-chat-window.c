...
empathy_chat_window_init (EmpathyChatWindow *window)
{
	...
	GtkWidget            *chat_vbox;
#ifdef USE_HILDON
        static gboolean      register_glade=FALSE;
#endif
...
#ifdef USE_HILDON
                                       "chats_mainmenu", &priv->menu_chats_mainmenu,
#endif
				       "chat_vbox", &chat_vbox,
				       "menu_conv", &menu_conv,
...
#ifdef USE_HILDON
        hildon_program_add_window (hildon_program_get_instance (), HILDON_WINDOW (priv->dialog));
        hildon_window_set_menu(HILDON_WINDOW(priv->dialog), GTK_MENU(priv->menu_chats_mainmenu));
#endif
