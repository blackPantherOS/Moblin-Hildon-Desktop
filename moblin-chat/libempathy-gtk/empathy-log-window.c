...
#ifdef USE_HILDON
	GtkWidget	  *moko_find;
        GtkWidget         *scrolledwindow_find_found;
        GtkWidget         *moko_find_found;
#endif        
...
#ifdef USE_HILDON
	GtkWidget	  *moko_chats;
        GtkWidget         *scrolledwindow_chats_found;
        GtkWidget         *moko_chats_found;
#endif        
...
#ifdef USE_HILDON
static GtkWidget* glade_hildon_calendar_new(GladeXML *xml, GType type, GladeWidgetInfo *info)
{
        return hildon_calendar_new();
}
#endif
			...
			GtkWindow   *parent)
{
...
#ifdef USE_HILDON
        glade_register_widget(HILDON_TYPE_CALENDAR,
                              glade_hildon_calendar_new,
...
#ifdef USE_HILDON
				       "scrolledwindow_find_found", &window->scrolledwindow_find_found,
#else
...
#ifdef USE_HILDON
				       "scrolledwindow_chats_found", &window->scrolledwindow_chats_found,
#else
	...
	/* We set this up here so we can block it when needed. */
        ...
        g_signal_connect (window->calendar_chats, "month-changed",
                          ...
                          window);
#else
        g_signal_connect (window->calendar_chats, "day_selected",
                          G_CALLBACK (log_window_calendar_chats_day_selected_cb),
                          window);
        g_signal_connect (window->calendar_chats, "month_changed",
                          G_CALLBACK (log_window_calendar_chats_month_changed_cb),
                          window);
        hildon_calendar_set_display_options(HILDON_CALENDAR(window->calendar_chats),
                                                HILDON_CALENDAR_SHOW_HEADING|HILDON_CALENDAR_SHOW_DAY_NAMES);
        gtk_widget_show(window->calendar_chats);
#endif
...
#else
        window->moko_find = moko_finger_scroll_new ();
        gtk_widget_show (window->moko_find);
        gtk_container_add (GTK_CONTAINER(window->moko_find),
                           GTK_WIDGET(window->chatview_find));
        gtk_container_add (GTK_CONTAINER (window->scrolledwindow_find),
                           window->moko_find);
        gtk_widget_show (GTK_WIDGET (window->chatview_find));
...
#else
        window->moko_chats = moko_finger_scroll_new ();
        gtk_widget_show (window->moko_chats);
        gtk_container_add (GTK_CONTAINER(window->moko_chats),
                           GTK_WIDGET(window->chatview_chats));
        gtk_container_add (GTK_CONTAINER (window->scrolledwindow_chats),
                           window->moko_chats);
        gtk_widget_show (GTK_WIDGET (window->chatview_chats));
...
#ifdef USE_HILDON
	/* configure scrolledwindow for treeview_find and treeview_chats*/
	window->treeview_find = gtk_tree_view_new ();
        window->moko_find_found = moko_finger_scroll_new ();
        gtk_widget_show (window->moko_find_found);
        gtk_container_add (GTK_CONTAINER(window->moko_find_found),
                           GTK_WIDGET(window->treeview_find));
        gtk_container_add (GTK_CONTAINER (window->scrolledwindow_find_found),
                           window->moko_find_found);
        gtk_widget_show (GTK_WIDGET (window->treeview_find));

	window->treeview_chats = gtk_tree_view_new ();
        window->moko_chats_found = moko_finger_scroll_new ();
        gtk_widget_show (window->moko_chats_found);
        gtk_container_add (GTK_CONTAINER(window->moko_chats_found),
                           GTK_WIDGET(window->treeview_chats));
        gtk_container_add (GTK_CONTAINER (window->scrolledwindow_chats_found),
                           window->moko_chats_found);
        gtk_widget_show (GTK_WIDGET (window->treeview_chats));
#endif
...
#else
        hildon_calendar_clear_marks (HILDON_CALENDAR (window->calendar_chats));
#endif
	...
	if (!date) {
		...
		for (l = dates; l; l = l->next) {
			...
			sscanf (str, "%4d%2d%2d", &year, &month, &day);
                        ...
                        gtk_calendar_get_date (GTK_CALENDAR (window->calendar_chats),
                                               ...
                                               NULL);
#else
                        hildon_calendar_get_date (HILDON_CALENDAR (window->calendar_chats),
                                               &year_selected,
                                               &month_selected,
                                               NULL);
#endif
...
#else
                        hildon_calendar_mark_day (HILDON_CALENDAR (window->calendar_chats), day);
#endif
			...
			day_selected = TRUE;
                        ...
                        hildon_calendar_select_day (HILDON_CALENDAR (window->calendar_chats), day);
#endif
		}
		...
		if (!day_selected) {
			/* Unselect the day in the calendar */
                        ...
                        hildon_calendar_select_day (HILDON_CALENDAR (window->calendar_chats), 0);
#endif
		}
	} else {
...
#else
        hildon_calendar_clear_marks (HILDON_CALENDAR (calendar));
#endif
	if (!log_window_chats_get_selected (window, &account,
					    &chat_id, &is_chatroom)) {
