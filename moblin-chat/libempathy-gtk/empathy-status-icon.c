...
static void       status_icon_new_message_cb      (GtkWidget              *widget,
						   EmpathyStatusIcon      *icon);
#ifdef USE_HILDON
static void       status_icon_edit_accounts_cb    (GtkWidget              *widget,
                                                   EmpathyStatusIcon      *icon);
#endif
                         ...
                         EmpathyStatusIcon *icon)
{
        ...
        if (priv->events) {
                ...
                gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (priv->show_window_item),
                                        show);
                gtk_widget_set_sensitive (priv->show_window_item,
                                          !empathy_accounts_dialog_is_running ());
...
#ifdef USE_HILDON
                              "tray_edit_accounts", "activate", status_icon_edit_accounts_cb,
#endif
...
#ifdef USE_HILDON
static void
status_icon_edit_accounts_cb (GtkWidget         *widget,
                              EmpathyStatusIcon *icon)
{
        EmpathyStatusIconPriv *priv;
...
{
#ifdef USE_HILDON
        if (FALSE == empathy_accounts_dialog_is_running ())
#endif
		status_icon_toggle_visibility (icon);
}
...
#ifndef USE_HILDON
        event = status_icon_event_new (icon, GTK_STOCK_DIALOG_QUESTION, (gchar *)str);
#else
        event = status_icon_event_new (icon, "empathy", (gchar *)str);
#endif
