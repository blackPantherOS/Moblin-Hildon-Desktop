...
real_init (NMDevice *dev)
{
	...
	nm_device_802_11_wireless_set_mode (self, IW_MODE_ADHOC);
	...
	nm_info ("%s: About to SET IWMODE.", iface);
		...
		if (iw_set_ext (nm_dev_sock_get_fd (sk), iface, SIOCSIWMODE, &wreq) == 0){
			if (mode != IW_MODE_ADHOC)
				...
				nm_info("setting card %s to channel 11 for ad-hoc networking", iface);
				// Set wreq's frequency to 11, see wireless.h
				wreq.u.freq.m = 11;
				wreq.u.freq.e = 0;
		...
		{
                       nm_info("Raji errno returned is %d %x\n",errno,errno);
			if (errno != ENODEV)              
			{
...
supplicant_interface_init (NMDevice80211Wireless *self)
{
	...
	kernel_driver = nm_device_get_driver (NM_DEVICE (self));
        ...
        */
      // Raji Commented out 
	/*if (!nm_null_safe_strcmp (kernel_driver, "ath_pci"))
               wpa_driver = "madwifi";
        else if (!nm_null_safe_strcmp (kernel_driver, "hostap_pci") || !nm_null_safe_strcmp (kernel_driver, "hostap_cs") || !nm_null_safe_strcmp (kernel_driver, "hostap_plx"))
               wpa_driver = "hostap";
	else*/
               wpa_driver = "wext";
                                ...
                                NMActRequest *req)
{
	...
	if (!nm_null_safe_strcmp (kernel_driver, "orinoco_cs"))
               	ap_scan = "AP_SCAN 2";
