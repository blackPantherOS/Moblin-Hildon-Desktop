...
hd_wm_class_init (HDWMClass *hdwm_class)
{
  ...
  hdwm_signals[HDWM_ACTIVE_WINDOW_UPDATE] =
	g_signal_new("active-window-update",
                     G_OBJECT_CLASS_TYPE(object_class),
                     G_SIGNAL_RUN_LAST,0,
                     NULL, NULL,
                     g_cclosure_marshal_VOID__VOID,
...
hd_wm_switch_info_window (HDWM *hdwm, HDWMEntryInfo *app_info, gboolean to_next)
{
  ...
  HDWMWindow *window;
  const GList *list = NULL, *iter, *new_entry;
  ...
  for (iter = list; iter != NULL; iter = g_list_next (iter))
  {
    ...
    if (window == hdwm->priv->active_window)
    {
      ...
      if (to_next)
      {
        ...
        if (!new_entry)
        {
          new_entry = g_list_first((GList *)list);
        }
      }
      else
      {
        ...
        if (!new_entry)
        {
          new_entry = g_list_last((GList *)list);
        }
      }
	
      info = (HDWMEntryInfo *) new_entry->data;
