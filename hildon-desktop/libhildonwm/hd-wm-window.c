			  ...
			  HDWMApplication *app)
{
  ...
  */
  ...
  win->priv->dpy        = XOpenDisplay(getenv("DISPLAY"));

  if(NULL == win->priv->dpy) {
     g_debug("libhildonwm: cannot open display! check your DISPLAY variable.\n");
  }
...
Display*
hd_wm_window_get_display (HDWMWindow *win)
{
  return win->priv->dpy;
}
