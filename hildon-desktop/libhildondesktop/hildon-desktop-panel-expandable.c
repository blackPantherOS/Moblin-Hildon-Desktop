...
#define HSB_ITEM_WIDTH 52
#define HSB_ITEM_HEIGHT 52
...
static gboolean hildon_desktop_panel_applet_expose_callback (StatusbarItemSocket *sb_socket, GdkEventExpose *event, gpointer data);
#endif
...
hildon_desktop_panel_expandable_init (HildonDesktopPanelExpandable *panel)
{
  ...
  panel->priv->launched_applets = NULL;
#endif
}
...
hildon_desktop_panel_remove_embed (GtkSocket *socket, HildonDesktopPanelExpandable *panel)
{
  ...
  if (panel->priv->launched_applets != NULL)
  {
    GList *iter;

    for (iter = panel->priv->launched_applets; iter ; iter = g_list_next (iter))
    {
      StatusbarItemSocket *applet_socket = (StatusbarItemSocket *)(iter->data);
      if (applet_socket->wid == ((StatusbarItemSocket *)parent)->wid)
        panel->priv->launched_applets = g_list_remove (panel->priv->launched_applets, applet_socket);
    }
  }
...
hildon_desktop_panel_embed_applet (HildonDesktopPanelExpandable *panel, Window wid)
{
  ...
  gtk_widget_set_app_paintable (GTK_WIDGET (sb_socket), TRUE); 
  ...
  g_signal_connect (G_OBJECT (sb_socket),
                    "expose-event",
                    G_CALLBACK (hildon_desktop_panel_applet_expose_callback),
                    panel);
...
static gboolean
hildon_desktop_panel_applet_expose_callback (StatusbarItemSocket *sb_socket, GdkEventExpose *event, gpointer data)
{
  HildonDesktopPanelExpandable *panel = (HildonDesktopPanelExpandable *)data;
  ...
  if (panel->priv->launched_applets != NULL)
  {
    ...
    {
      StatusbarItemSocket *applet_socket = (StatusbarItemSocket *)(iter->data);
      if (applet_socket->wid == sb_socket->wid)
        return FALSE;
    }
  }

  if (event->window == GTK_WIDGET (sb_socket)->window)
  {
    g_print ("hildon_desktop_panel_applet_expose_callback\n");
    if (GTK_WIDGET_REALIZED (GTK_WIDGET (sb_socket->socket)))
      gtk_widget_hide (GTK_WIDGET (sb_socket->socket));
    else
      gtk_widget_realize (GTK_WIDGET (sb_socket->socket));

    gdk_window_set_back_pixmap (GTK_WIDGET (sb_socket->socket)->window, NULL, TRUE);
    gdk_window_clear (GTK_WIDGET (sb_socket->socket)->window);
    gtk_widget_show (GTK_WIDGET (sb_socket->socket));

    panel->priv->launched_applets = g_list_append (panel->priv->launched_applets, sb_socket);
  }
  else
    g_print ("this is an expose event to other windows, ignore this\n");
