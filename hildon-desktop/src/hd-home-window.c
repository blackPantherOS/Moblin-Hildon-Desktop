...
#define HD_DESKTOP_USER_PATH                ".config/hildon-desktop/"
...
#define HH_AREA_CONFIGURATION_FILE        ".config/hildon-desktop/home-layout.conf"
#define HH_AREA_GLOBAL_CONFIGURATION_FILE "/etc/xdg/hildon-desktop/home-layout.conf"
...
static void
hd_home_window_set_is_background_cb (GtkWidget *widget,
                                     gpointer data);
...
hd_home_window_set_is_background_cb (GtkWidget *widget,
                                     gpointer data)
{
  HildonDesktopHomeItem *item = HILDON_DESKTOP_HOME_ITEM (widget);
  gboolean is_background = (gboolean)GPOINTER_TO_INT (data);

  hildon_desktop_home_item_set_is_background (item, is_background);
}
  ...
  if (!priv->screen_is_off)
    {
      ...
      gtk_container_foreach (GTK_CONTAINER (area),
                             (GtkCallback)hd_home_window_set_is_background_cb,
