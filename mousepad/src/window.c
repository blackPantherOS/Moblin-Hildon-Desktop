	...
	osso_rpc_t * retval)
{
	...
	osso_system_note_infoprint(mainwin->osso_context, method, retval);
...
MainWindow *create_main_window(StructData *sd)
{
	...
	mainwin->osso_context = osso_initialize(OSSO_NOTES_SERVICE, OSSO_NOTES_VERSION, TRUE, NULL);
	if (NULL == mainwin->osso_context)
		return OSSO_ERROR;
	...
	g_set_application_name("Notes");

	mainwin->program = program;
	mainwin->window = window;

	/* Add handler for session bus D-BUS messages */
	osso_rpc_set_default_cb_f (mainwin->osso_context, 
	...
	menubar = create_menu_bar(GTK_WIDGET(window), sd);
