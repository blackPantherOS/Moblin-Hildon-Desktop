...
static CharsetTable *get_charset_table(void)
{
	...
	if (!ctable) {
		...
		ctable->str[ctable->num] = g_strdup_printf("Current Locale (%s)", get_default_charset());
		ctable->num++;
...
static GtkWidget *init_menu_item_manual_charset(gchar *manual_charset)
{
	...
	other_codeset_title = "Other Codeset";
...
static gboolean get_manual_charset(GtkOptionMenu *option_menu, FileInfo *selected_fi)
{
	...
	label = gtk_label_new_with_mnemonic("Code_set:");
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
		...
		if (err) {
			...
			str = g_strdup_printf("'%s' is not supported", gtk_entry_get_text(GTK_ENTRY(entry)));
...
static GtkWidget *create_charset_menu(FileInfo *selected_fi)
{
	...
	if (mode == OPEN) {
		menu_item = gtk_menu_item_new_with_label("Auto Detect");
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
		gtk_widget_show(menu_item); /* <- required for width adjustment */
	}
	ctable = get_charset_table();
...
static GtkWidget *create_file_chooser(FileInfo *selected_fi, GtkWidget *window)
{
	...
	gtk_window_set_position(GTK_WINDOW(filesel), GTK_WIN_POS_CENTER_ALWAYS);
	...
	label = gtk_label_new_with_mnemonic("Ch_aracter Coding: "); /* dummy for pot */
	...
	do {
		...
		if (res == GTK_RESPONSE_ACCEPT) {
			...
			if (mode == SAVE && g_file_test(selected_fi->filename, G_FILE_TEST_EXISTS)) {
				...
				str = g_strdup_printf("'%s' already exists. Overwrite?", basename);
				res = run_dialog_message_question(filesel, "Overwrite", str);
				g_free(str);
				g_free(basename);
				switch (res) {
