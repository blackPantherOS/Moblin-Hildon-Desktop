...
/*	undo_disconnect_signal(textbuffer); */
  ...
  recent_data.app_name = "Moblin Notes Text Editor";
  recent_data.app_exec = "notes %f";
...
gint file_save_real(GtkWidget *textview, FileInfo *fi)
{
	...
	if (err) {
		...
		case G_CONVERT_ERROR_ILLEGAL_SEQUENCE:
			run_dialog_message(gtk_widget_get_toplevel(textview),
				GTK_MESSAGE_ERROR, "Can't convert codeset to '%s'", fi->charset);
			break;
		default:
			run_dialog_message(gtk_widget_get_toplevel(textview),
	...
	if (!fp) {
		run_dialog_message(gtk_widget_get_toplevel(textview),
			GTK_MESSAGE_ERROR, "Can't open file to write");
		return -1;
	}
	if (fputs(str, fp) == EOF) {
		run_dialog_message(gtk_widget_get_toplevel(textview),
			GTK_MESSAGE_ERROR, "Can't write file");
		return -1;
	}
