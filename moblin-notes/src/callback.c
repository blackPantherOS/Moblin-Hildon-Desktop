...
static gint check_text_modification(StructData *sd)
{
	...
	if (gtk_text_buffer_get_modified(textbuffer)) {
		...
		str = g_strdup_printf("Save changes to '%s'?", basename);
		res = run_dialog_message_question(GTK_WIDGET(sd->mainwin->window), "Changes Not Saved", str);
		g_free(str);
		g_free(basename);
		switch (res) {
...
/*		fi = get_file_info_by_selector(OPEN, sd->fi); */
		window = gtk_widget_get_toplevel(GTK_WIDGET(sd->mainwin->textview));
		fi = get_fileinfo_from_chooser(window, sd->fi, OPEN);
		if (fi) {
...
gint cb_file_save_as(StructData *sd)
{
	...
	fi = get_fileinfo_from_chooser(GTK_WIDGET(sd->mainwin->window), sd->fi, SAVE);
	if (fi) {
...
gint cb_file_print(StructData *sd)
{
	...
	if (conv_err) {
		...
		case G_CONVERT_ERROR_ILLEGAL_SEQUENCE:
			run_dialog_message(gtk_widget_get_toplevel(sd->mainwin->textview),
				GTK_MESSAGE_ERROR, "Can't convert codeset to '%s'", sd->fi->charset);
			break;
		default:
			run_dialog_message(gtk_widget_get_toplevel(sd->mainwin->textview),
	...
	if (pipe_err) {
		run_dialog_message(gtk_widget_get_toplevel(sd->mainwin->textview),
			GTK_MESSAGE_ERROR, "Can't open pipe to process");
		g_error_free(pipe_err);
		return -1;
	}

	gtk_widget_set_sensitive(GTK_WIDGET(sd->mainwin->window), FALSE);
	...
	gtk_widget_set_sensitive(GTK_WIDGET(sd->mainwin->window), TRUE);
...
{
	run_dialog_about(GTK_WIDGET(sd->mainwin->window),
		PACKAGE_NAME,
		PACKAGE_VERSION,
