...
static gint document_replace_real(StructData *sd)
{
	...
	if (!sd->search.replace_all)
		q_dialog = create_dialog_message_question(
			gtk_widget_get_toplevel(sd->mainwin->textview), "Replace", "Replace?");

	do {
	        res = gtk_source_iter_forward_search(
        	        &iter, sd->search.string_find, search_flags, &match_start, &match_end, NULL);
		
		// When searching reaches the end of textbuffer, we need to search from beginning
		// again, this is for loop back searching.
		if(!res)
		{
			gtk_text_buffer_get_start_iter(textbuffer, &iter);
			res = gtk_source_iter_forward_search(
		...
		gtk_text_buffer_delete(textbuffer, &match_start, &match_end);
...
gint run_dialog_find(StructData *sd)
{
	...
	check_case = gtk_check_button_new_with_mnemonic("_Match case");
		...
		if (res == GTK_RESPONSE_OK) {
			...
			sd->search.string_find = g_strdup(gtk_entry_get_text(GTK_ENTRY(entry_find)));
	                ...
	                if (strlen(sd->search.string_find)) {
        	                result = document_search_real(sd, 0);
                	}

		}

                if ((res == GTK_RESPONSE_CANCEL) || (FALSE == result))
                {
...
void run_dialog_jump_to(StructData *sd)
{
	...
	dialog = gtk_dialog_new_with_buttons("Jump To",
		...
		GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
		NULL);
	button = gedit_button_new_with_stock_image("_Jump", GTK_STOCK_JUMP_TO);
